...
export function AttributionLedger() {
  ...
  return (
    <aside className="flex h-full w-80 shrink-0 flex-col gap-4 overflow-auto border-l border-slate-800 bg-slate-950 p-3">
      <section>
            ...
            <AnimatePresence initial={false}>
              {attributions.map((a) => (
                <motion.div
                  ...
                  className="rounded border border-slate-700 bg-slate-900/70 p-2.5"
                >
                  <div className="flex items-baseline justify-between">
                    <span className="font-mono text-xs text-slate-300">{a.task_id}</span>
                    <span className="font-mono text-xs text-red-400">+{a.slip_days}d</span>
                  </div>

                  <div className="mt-1.5 grid grid-cols-2 gap-x-2 gap-y-0.5 text-[10px] text-slate-500">
                    <span>Float consumed</span>
                    <span className="text-right text-slate-300">{a.float_consumed}d</span>
                    <span>Downstream</span>
                    <span className="text-right text-slate-300">
                      {a.downstream_affected.length} tasks
                    </span>
                    <span>Project slip</span>
                    <span className="text-right text-red-400">+{a.project_slipped_days}d</span>
                  </div>
          ...
          {purchaseOrders.map((po) => (
            ...
            >
              <div className="flex justify-between">
                ...
                <span
                  className={
                    po.status === 'rescheduled' ? 'text-sky-300' : 'text-slate-500'
                  }
                >
                  {po.status}
                </span>
              ...
              {po.last_action && (
                <p className="mt-1 text-sky-300/80">{po.last_action}</p>
              )}
            </motion.div>
          ))}
        </div>
